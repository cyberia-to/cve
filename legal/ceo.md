---
tags: cyber valley, cyberia, legal, ceo, offer, draft, dzin
alias: cve ceo, ceo offer, ceo agreement, ceo pack
crystal-type: entity
crystal-domain: socio
crystal-size: article
icon: "🧭"
---
# CEO — PT Cyber Valley Estate

> **Offer / agreement (one text).** Draft for discussion until signed.  
> No separate economics annex, no open-items sheet, no long legal novel.

**Company.** PT Cyber Valley Estate ([[pma|PMA]]).  
**Mandate.** CEO who grows a **network of cities in Indonesia**, starting from **~37 ha** on **Sanghyang** (Bali): **Bali North Star / Cyber Valley** — nature × technology; mountain tourism + quality of life as tech hub.

**Job in one line.** Capitalise the structure and **decentralise ownership** so **founders (DS + AV) residual ≈ 10%**, with blocks in Indonesian hands, foreign hands (Cyberia holding), and residual for founders + CEO — without wrecking valuation.

---

## 0. Entry — appointment and 1% are **one deal**

Cannot unglue:

| leg | instrument |
|---|---|
| **Buy 1% (80 shares) at par** from founder | [[share sale entry]] |
| **Appoint as President Director** | [[gms director appointment]] |

**Closing rule:** buy completes ↔ appointment takes effect.  
No completed purchase → **no President Director**. No appointment → **sale does not complete**.

Same-day package with [[shareholders agreement]] accession as needed.  
This page is **how the CEO works and is paid in options + ladder** after entry. Entry itself is GMS + share sale.

---

## 1. Role

| | |
|---|---|
| **Strategic** | Direction, capitalisation, city network, investors |
| **Operational** | Day-to-day CVE inside dual-account and commissioner rules below |

**Success metric.** Sales of **DS + AV** founder equity on the ladder in [§7](#7-kpi-ladder-ds-av-10-founders-company-1b). No other KPI stack.

Fixed monthly salary, if any, is an **ops proposal** (budget), **not** a term of this deal.

---

## 2. Dual accounts

| account | purpose |
|---|---|
| **Strategic** | All **incoming revenue**; strategic reserve |
| **Operational** | Day-to-day spend (payroll, suppliers, ops) |

Revenue lands on **strategic** only. No silent mixing.

---

## 3. Strategic gate (monthly)

Once per month (or written circular):

- CEO tables **proposals** (props) for strategic spend and material decisions  
- **One commissioner** approval is enough  
- Ops inside the approved monthly envelope needs no commissioner each time  

---

## 4. Ops funding (monthly)

- **One** transfer strategic → operational per month  
- Amount = envelope from the [§3](#3-strategic-gate-monthly) pack  
- Overspend / new lines → back to [§3](#3-strategic-gate-monthly)  

---

## 5. Land and credit — hard limits

| rule | |
|---|---|
| **Below market** | Any **[[hgb|HGB]]** or **hak sewa** below market needs **commissioner** approval |
| **Market floor** | **USD 3,000 / are (sotka)** — under this = below market unless commissioner signs |
| **No mortgage** | CEO **cannot** pledge or mortgage Company (or controlled [[spv|SPV]]) **real estate** for loans |

---

## 6. Options — up to 10% at $4M (sale from founders only)

| | |
|---|---|
| **Strike basis** | Company valuation **USD 4,000,000** (1% ≈ USD 40,000) |
| **Pool** | up to **10%** of the company |
| **Tranche A** | **5%** · exercise within **2 years** of start |
| **Tranche B** | **5%** · exercise within **7 years** of start |
| **Independence** | Tranches are **independent** — missing the Tranche A window does **not** forfeit or block Tranche B |
| **Rescheduling** | Either window may be extended or shortened **by written mutual consent** of CEO and founders |
| **Source** | **only sale/transfer from founders** — **no new share issue** for this package |
| **[[rofr|ROFR]]** | Transfers of option shares under this [§6](#6-options-up-to-10-at-4m-sale-from-founders-only) follow [[shareholders agreement]] Article 1: the other Shareholders have a **3 (three) day** window to take up the shares first; once it passes unexercised, the transfer to the CEO proceeds |

Options ≠ KPI ladder ([§7](#7-kpi-ladder-ds-av-10-founders-company-1b)). Personal upside for the chair; shares come from founders’ holdings, not dilution by print.

---

## 7. KPI ladder — DS + AV → ~10% founders · company → **$1B**

### Counts

| counts | does not |
|---|---|
| Sale/transfer of **DS** and **AV** at real price | Free print of other share classes |
| Cash + valuation on those sales | Cash-out theatre without third-party price |
| | Capital raise that does not move DS/AV on this ladder |

### Package split (every step **except seed**)

| | |
|---|---|
| **Package** | `step` of company equity leaving founders |
| **CEO** | **10% × step** (equity) |
| **Buyers** | **90% × step** (pay cash) |
| **Cash** | `0.90 × step × val` |

**Seed (1% @ $4M):** no CEO cut · cash = `1% × val` · not a CEO sale.  
**First CEO sale:** **10%** cum. @ **$10M**.

### Target hands

| block | |
|---|---|
| Package 1 | Indonesian holders |
| Package 2 | Foreign / Cyberia holding |
| Residual ~10% | Founders |
| CEO | ladder ~**8.9%** + options ≤10% @ $4M from founders |

### Table

| sold cum. | step | val (USD) | CEO step | CEO cum. | cash step (USD) |
|---:|---:|---:|---:|---:|---:|
| **1%** | 1% | **4,000,000** | **—** | **0%** | **40,000** |
| **10%** | 9% | **10,000,000** | **0.90%** | **0.90%** | **810,000** |
| **20%** | 10% | **18,000,000** | **1.00%** | **1.90%** | **1,620,000** |
| **30%** | 10% | **30,000,000** | **1.00%** | **2.90%** | **2,700,000** |
| **40%** | 10% | **50,000,000** | **1.00%** | **3.90%** | **4,500,000** |
| **50%** | 10% | **90,000,000** | **1.00%** | **4.90%** | **8,100,000** |
| **60%** | 10% | **160,000,000** | **1.00%** | **5.90%** | **14,400,000** |
| **70%** | 10% | **280,000,000** | **1.00%** | **6.90%** | **25,200,000** |
| **80%** | 10% | **500,000,000** | **1.00%** | **7.90%** | **45,000,000** |
| **90%** | 10% | **1,000,000,000** | **1.00%** | **8.90%** | **90,000,000** |

Miss the val floor → step does not count (unless founders waive in writing).  
Between rows: log-interpolate val.

---

## 8. Exit

Shares held by the CEO are ordinary shares under the [[articles of association]] and the [[shareholders agreement]] — [[rofr|ROFR]] and tag-along apply to any sale, the same as for every shareholder.

| trigger | rule |
|---|---|
| **Removal from Director office** (by resolution of the shareholders) | Removal takes effect **immediately**. Bought back are only the shares the CEO **bought from the founders** — the entry stake and any exercised option shares alike (options under [§6](#6-options-up-to-10-at-4m-sale-from-founders-only)) — each valued at the **company valuation at the date of removal** (determined under [§18](#18-valuation-methodology)), with a floor of **USD 40,000 per 1% held**, being the price he paid for it. Shares transfer to the buyer in tranches matching the payment schedule in "Payment of the buyout" below, not on removal itself — see that row and, for the entry stake, the [[entry payment side letter]]. Cause of removal does not change the price. |
| **What stays with the CEO** | Ladder equity earned from closed steps under [§7](#7-kpi-ladder-ds-av-10-founders-company-1b) is **not** bought out — it stays his, as ordinary shares. Commissions and referral fees under [§17](#17-survival-after-departure) stay his on their own terms. Unexercised options lapse. |
| **The valuation** | Determined under [§18](#18-valuation-methodology). |
| **Payment amount & schedule** | A debt of the buyer, not a condition of removal. If the CEO agrees, the amount is fixed at **USD 40,000 per 1% held** without a valuation, and instalments start immediately on that agreement. Absent CEO agreement, the amount is determined under [§18](#18-valuation-methodology) — fixed in US dollars as of the removal date (the §18 valuation date) — and instalments start as soon as that amount is finalised. Either way the amount is paid in **12 equal moonly instalments**, and the bought-back shares transfer to the buyer in the same twelve tranches — each tranche consisting of one-twelfth of the total bought-back shares, rounded down, with the remainder carried to the final tranche — released against its matching instalment. Until each tranche transfers, the CEO remains the registered holder of, and keeps all rights attaching to, the shares not yet transferred. On the fast-track, the shares transfer in full against that single payment. |
| **Currency, default & security** | Each instalment is converted to IDR at [JISDOR](https://www.bi.go.id/en/statistik/informasi-kurs/jisdor/default.aspx) on its payment date, as [UU 7/2011](/cyber-valley/cve/legal/laws/uu-7-2011-mata-uang) requires — the dollar figure does not move, only the conversion does, so the CEO carries no rupiah devaluation risk. Late payment — any instalment not paid within **5 days of its due date**, automatically and with no notice required — accelerates the entire remaining balance, whether owed by the buyer under this section, by a Founder under the [[entry payment side letter]], or by a Founder as an option-share shortfall obligor. **Primary, secured layer**: a dedicated escrow held with an independent notary and/or a bank guarantee or standby LC in the CEO's favour, established by GMS resolution before the first instalment under this section falls due, at a threshold equal to the then-current guaranteed floor (USD 40,000 per 1% held, recalculated at each Annual Reference Valuation under [§18](#18-valuation-methodology)). **Funding**: while the escrow balance is below that threshold, the Company sweeps 5% (five percent) of each revenue payment received — excluding capital contributions, loan proceeds, and transfers between the Company's own accounts — into the escrow within 5 (five) business days of receipt, through the maker-and-authorizer mechanism required for every disbursement under Articles of Association Pasal 12(4); the sweep resumes whenever the threshold rises at an Annual Reference Valuation, or whenever a payout draws the escrow back down, until it again meets the current threshold. The escrowed funds are held by the notary, not as an entry on the Company's own books, so their availability does not depend on the Company's own signatures or solvency. **Enforcement**: on presentation of either a BANI arbitral award or a default certificate the CEO signs alone — no counter-signature or further act by the Company required. **Secondary, unsecured covenant**: for any part of an accelerated balance not recovered from that instrument, the Company must apply, in order — (i) available unrestricted Company funds; (ii) the Company's buyback capacity under Pasal 37 of [UU 40/2007](/cyber-valley/cve/legal/laws/uu-40-2007-perseroan-terbatas), applied to acquire the relevant shares directly — ⚠ capped by that statute at 10% of the Company's issued capital in aggregate across all treasury holdings, so available only to the extent that ceiling is not already used elsewhere; any shortfall this layer cannot cover for that reason passes to (iii); (iii) the reserve maintained under [[shareholders agreement]] (reserve allocation) — ⚠ that reserve is the mandatory reserve under Pasal 70 of [UU 40/2007](/cyber-valley/cve/legal/laws/uu-40-2007-perseroan-terbatas); confirm with counsel that it may be applied to a share buyback rather than only to loss-absorption before relying on it; (iv) a loan secured against Company-owned land, arranged independently of any security given to other creditors. Payment by the Company from either layer subrogates it to the CEO's claim against the defaulting Founder for the amount paid. This entire mechanism — amount, schedule, currency conversion, share-transfer tranches, security and covenant — applies equally to the amount payable under the [[entry payment side letter]] on removal (its paragraph 3). |
| **CEO's own decision to leave** | No buyout. The CEO keeps the equity already his — ladder equity earned from closed steps, options per time earned, and the entry stake — and may keep the stake and continue to receive dividends, or offer the shares to the founders and other shareholders first, then to third parties, under the [[rofr|ROFR]] in the [[shareholders agreement]]. |

---

## 9. Not in this deal

Land commission grids, sub-project referral stacks, investment-raise %, DevelopmentCo menus, multi-page indemnification, fixed salary as a negotiated term (ops budget only) — terms live in separate documents, but survival on departure is confirmed by [§17](#17-survival-after-departure).

---

## 10. Acceptance

| party | role | sign / date |
|---|---|---|
| PT Cyber Valley Estate | company; backstops any Founder buyout shortfall under [§8](#8-exit) ("Currency, default & security") if the Founder does not pay within the cure period | |
| Commissioners | strategic gate | |
| Founders (DS / AV) | option sellers; primary obligors for any buyout shortfall under [§8](#8-exit); backstopped by the Company on the same cure-period, acceleration, security-then-covenant and subrogation terms as the entry-stake repayment | |
| CEO | offer | |

Draft until signed.

---

## 11. Law & disputes

| | |
|---|---|
| **Governing law** | Republic of Indonesia — same as [[articles of association]] and [[shareholders agreement]] |
| **Disputes** | Good-faith negotiation first; unresolved after 30 days → arbitration under [[bani|BANI]] rules, seat Bali, in English |

---

## 12. Pre-appointment liability

| | |
|---|---|
| **Shield** | CEO bears no contractual liability to the Company or Shareholders for acts, obligations, breaches or facts arising **before** the date of appointment, unless CEO personally participated in them |
| **Basis** | Company and Shareholders confirm disclosure to CEO of all known material debts, contracts (incl. MoUs, side letters, material oral arrangements), guarantees, judicial/administrative proceedings, tax exposure, land encumbrances, related-party arrangements and other liabilities as of the appointment date |

---

## 13. Legal defence — narrow indemnification

| | |
|---|---|
| **Scope** | Company advances CEO's reasonable legal defence costs, and indemnifies CEO, for third-party claims (incl. tax penalties/interest) traceable to (a) facts pre-dating appointment, (b) inaccuracy of the [§12](#12-pre-appointment-liability) disclosure, or (c) an error or omission by the Company's appointed accountant/tax adviser where CEO relied on them in good faith with no actual knowledge of the error — to the extent permitted by Indonesian law, save for CEO's own fraud, wilful misconduct or gross negligence |
| **Advance, not reimbursement** | Costs are advanced as incurred, not repaid after the fact |
| **Carve-out from [§9](#9-not-in-this-deal)** | Narrows, not reopens, [§9](#9-not-in-this-deal)'s exclusion of "multi-page indemnification" — a single-purpose defence-cost clause, not general indemnity for CEO's own acts in office |

---

## 14. Right to refuse

| | |
|---|---|
| **Refusal** | CEO may decline to execute any instruction or sign any transaction that shows reasonable signs of illegality, lack of required corporate authority, or unjustified personal-liability risk to CEO |
| **Documentation** | Material shareholder instructions outside ordinary operations must be given in writing |

---

## 15. Anti-circumvention

| | |
|---|---|
| **Prohibited** | Unjustified dilution of CEO's stake; artificial suppression of company value ahead of a buyout or termination; below-market transfer of land, projects or other assets to related parties; manufactured debt or obligations — any act aimed at defeating CEO's economic rights under this page |
| **Remedy** | Value for [§6](#6-options-up-to-10-at-4m-sale-from-founders-only) options, [§7](#7-kpi-ladder-ds-av-10-founders-company-1b) ladder and [§8](#8-exit) buyout is computed **as if** the prohibited act had not occurred |

---

## 16. Change of control

| | |
|---|---|
| **Trigger** | Sale of the Company, or of a controlling stake, to a third party. Sales of land, plots or individual projects are the Company's ordinary business and are not a change of control |
| **CEO rights** | Tag-along already applies as an ordinary shareholder ([[shareholders agreement]] Art. 2); unvested [§6](#6-options-up-to-10-at-4m-sale-from-founders-only) options accelerate to fully vested; unbanked [§7](#7-kpi-ladder-ds-av-10-founders-company-1b) ladder steps in progress at signing are paid out at the transaction valuation |
| **Interaction with [§8](#8-exit)** | If removal from Director office occurs in connection with a change of control, the CEO is entitled to the **greater of** the [§16](#16-change-of-control) treatment above and the [§8](#8-exit) removal-buyout floor — not both cumulatively for the same shares. |

---

## 17. Survival after departure

| | |
|---|---|
| **Survival** | [§12](#12-pre-appointment-liability) shield and [§13](#13-legal-defence-narrow-indemnification) defence-cost advance continue after CEO's departure, for the periods they cover, regardless of when a claim is brought |
| **Acts in office** | Extends to CEO's good-faith acts within authority under [§14](#14-right-to-refuse) while in office, on the same defence-cost-advance basis as [§13](#13-legal-defence-narrow-indemnification) |
| **[[do|D&O]] cover** | Company maintains directors & officers insurance naming CEO, continuing for a tail period after departure |
| **Commissions & referral fees** | Land commission grids, investment-raise / capital-introduction %, and any other commission agreed with the CEO under the separate arrangements referenced in [§9](#9-not-in-this-deal) survive removal or departure in full, regardless of cause — payable on their own terms for any deal **signed before the date of departure**, whenever the money actually arrives. Removal from Director office does not reduce, delay beyond their own terms, or extinguish these amounts. |

---

## 18. Valuation methodology

| | |
|---|---|
| **Priority** | (a) price per share in the most recent bona fide arm's-length sale to an unaffiliated third party within **12 moon cycles** before the removal date — excluding sales to related parties, nominal/non-market consideration, and anything caught by [§15](#15-anti-circumvention); (b) failing that, the most recent Annual Reference Valuation (below), if dated within 12 moon cycles of the removal date; (c) failing both, the independent valuation below |
| **Appointment** | CEO and Founders each appoint one licensed valuer (KJPP or internationally recognised equivalent) within 14 days of the removal date; each delivers within 30 days of appointment |
| **Reconciliation** | Figures within 10% of the higher → their average. Figures more than 10% apart → the two valuers jointly appoint a third within 7 days (who delivers within 30 days); final value = average of the two closest of the three figures |
| **Default** | Either party misses the 14-day appointment window → the other party's valuer is binding. First two valuers can't agree a third within 7 days → appointed by MAPPI, or failing that [[bani|BANI]] |
| **Basis** | Under (a), the per-share price from that sale applies as agreed between its parties — no adjustment. Under (b) or (c): 100% of Company equity, going-concern, no discount for minority interest or lack of marketability; disregards any act caught by [§15](#15-anti-circumvention); includes a KJPP appraisal of the Land at current market value as a required component. Valuation date under any of the three = date of the removal resolution |
| **Cost** | Borne by the Company, including the Annual Reference Valuation below |
| **Annual Reference Valuation** | Company obtains an independent KJPP equity valuation at least once every **12 moon cycles**, timed with the annual GMS under Article 17 of the [[articles of association]], at the Company's cost — a running reference for the priority order above and any other purpose requiring a Company valuation |
